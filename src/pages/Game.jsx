...
const Game = () => {
  ...
  const [crops, setCrops] = useState([
    { id: 1, name: 'Rice', cost: 20, time: 300, icon: 'R', xp: 15 },
    { id: 2, name: 'Coconut', cost: 100, time: 1200, icon: 'C', xp: 50 },
    { id: 3, name: 'Spices', cost: 50, time: 600, icon: 'S', xp: 25 },
    { id: 4, name: 'Banana', cost: 30, time: 450, icon: 'B', xp: 20 },
  ]);
  ...
  const createManualGrid = () => {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (Math.random() > 0.7) {
        tile.innerHTML = ['P', 'G', 'C'][Math.floor(Math.random() * 3)];
  ...
  const handleTileClick = (tileId, tileElement) => {
            ...
            <Card>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs bg-primary text-white px-2 py-1 rounded-full">Done</span>
                </div>
                <div className="flex justify-between items-center">
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            {crops.map((crop) => (
              ...
              >
                <div className="text-2xl mb-2 font-bold bg-green-100 w-10 h-10 rounded-full flex items-center justify-center mx-auto">{crop.icon}</div>
                <p className="font-medium text-sm">{crop.name}</p>
                <p className="text-xs text-muted">Cost: {crop.cost} • XP: {crop.xp}</p>
                <p className="text-xs text-muted">Time: {Math.floor(crop.time / 60)}min</p>
              </button>
            ))}
          </div>
        </Modal>
